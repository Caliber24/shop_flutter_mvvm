// این کلاس لایه ارتباط با API را مدیریت می‌کند (HTTP client).
...
class ApiService {
  ...
  Future<List<Product>> fetchPopularProducts({int limit = 12}) async {
    final r = await get("/products?limit=$limit&select=title,price,rating,thumbnail");
    final List data = (r["products"] ?? []) as List<dynamic>;
    return data
        .map((e) => Product.fromJson(e as Map<String, dynamic>))
        .toList();
